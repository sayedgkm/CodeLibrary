...
///******************************************START******************************************
/// also we know form chinese remainder theorem
///if we know X%a and X%b ..... X%z then we know X%lcm(a,b...z)
class chineseReminder {
    ...
    ll findx(int n) { /// it will return X where X = r[i](%ar[i]);
        ...
        ll ans=0;
        for(int i=0; i<n; i++) {
            ll temp=product/ar[i];
            ans=(ans+(temp*r[i]*modularInverse(temp,ar[i])))%product;
        }
        return ans;
    }
}chinese;
