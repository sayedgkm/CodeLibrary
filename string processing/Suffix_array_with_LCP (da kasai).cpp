...
void Deal (string &s)
{
    ...
    for(int i =0;i<s.length();i++)
        data[i] = (int)s[i];
    data[n]=0;
    DA(data,saffix,n+1,128);
    calheight(data,saffix,n);
    for(int i = 0;i<n;i++) {
        saffix[i]=saffix[i+1];
        lcp[i] = lcp[i+1];
    }
    ...
    /// saffix is sorted suffix of given string
    ///lcp[i] = lcp(saffix[i],saffix[i-1])
}
