...
int go(int a,int b) {
    a = node[a];
    b = node[b];
    int dis = depth[a]+depth[b]-2*depth[lca_query(a,b)]-1;
    debug(a,b,dis);
    return max(dis,0);
...
int main(){
    ...
    while(1) {
        ...
        dfs(0,-1,0);
        lca_init(n);
        for(int i = 0;i<n;i++) debug(node[i]);
        int q = nxt();
        while(q--) {
            int i = nxt()-1;
            int j =  nxt()-1;
            int ans = 0;
            ans = max(ans,go(edge[i].ff,edge[j].ff));
            ans = max(ans,go(edge[i].ff,edge[j].ss));
            ans = max(ans,go(edge[i].ss,edge[j].ff));
            ans = max(ans,go(edge[i].ss,edge[j].ss));
            printf("%d\n",ans);
